...
st.set_page_config(page_title='A股/ETF 抄底雷达 V5', layout='wide')

POPULAR_STOCKS = {
    '601899':'紫金矿业','600519':'贵州茅台','300750':'宁德时代','002594':'比亚迪',
    '601318':'中国平安','600036':'招商银行','601088':'中国神华','600900':'长江电力',
    '600031':'三一重工','601138':'工业富联','000858':'五粮液','002371':'北方华创',
    '300308':'中际旭创','688981':'中芯国际',
}
POPULAR_ETFS = {
    '510300':'沪深300ETF','510050':'上证50ETF','510500':'中证500ETF','588000':'科创50ETF',
    '159915':'创业板ETF','512480':'半导体ETF','512660':'军工ETF','512800':'银行ETF',
    '515790':'光伏ETF','516160':'新能源ETF','159928':'消费ETF','518880':'黄金ETF',
}

def retry(fn, attempts=2, delay=0.8):
    err = None
...
def normalize_yf(df):
    ...
    for c in needed:
        if c not in x.columns:
            x[c] = np.nan
        x[c] = pd.to_numeric(x[c], errors='coerce')
    return x[['Date'] + needed].dropna(subset=['Close']).sort_values('Date').set_index('Date')

@st.cache_data(ttl=1800, show_spinner=False)
def load_hist(code, asset, years=4):
    start_dt = date.today() - timedelta(days=365*years + 50)
    ...
    try:
        ...
        df = retry(fetch_ak, attempts=2)
        if df is not None and not df.empty:
            out = normalize_ak(df)
            out.attrs['source'] = 'AKShare / 东方财富'
            return out
    except Exception as e:
        errors.append(f'AKShare: {type(e).__name__}')

    try:
        ys = yahoo_symbol(code)
        df = yf.download(
            ys,
        ...
        if df is not None and not df.empty:
            out = normalize_yf(df)
            out.attrs['source'] = f'Yahoo Finance ({ys})'
            return out
    except Exception as e:
        errors.append(f'Yahoo: {type(e).__name__}')

    raise RuntimeError('两个数据源都失败：' + ' | '.join(errors))
...
def analyze(code, name, asset, years=4):
    raw = load_hist(code, asset, years)
    source = raw.attrs.get('source','未知')
    h = features(raw)
    if len(h) < 80:
        raise ValueError('历史数据不足80个交易日')
    last = h.iloc[-1]
    ...
    return {
        '代码':code,'名称':name,'类型':asset,'数据源':source,
...
def display_table(df):
    show = df.copy()
    numeric_formats = {
        '最新价':'{:.3f}','涨跌%':'{:.2f}','步距σ':'{:.2f}','60日回撤%':'{:.1f}',
        'RSI14':'{:.1f}','RV20%':'{:.1f}','RV分位%':'{:.0f}'
    }
    st.dataframe(show.style.format(numeric_formats, na_rep='—'), use_container_width=True, hide_index=True)

st.title('🇨🇳 A股 / ETF 抄底雷达 V5')
st.success('云端稳定版：AKShare 失败时会自动切换 Yahoo Finance。')
st.caption('热门抄底榜｜自选扫描｜单票查询。当前以日线统计为主。')
    ...
    if st.button('▶ 生成热门抄底榜', type='primary'):
        ...
        for i,(code,name,asset) in enumerate(universe,1):
            ...
            try:
                ...
                failures.append(f'{name}({code}): {e}')
            bar.progress(i/total)
        status.empty()
        if rows:
            df = pd.DataFrame(rows).sort_values(['抄底分','步距σ'],ascending=False)
            display_table(df)
            st.caption(f'成功 {len(rows)} 只；失败 {len(failures)} 只。')
        else:
            st.error('所有股票都读取失败。')
        if failures:
...
with tab2:
    ...
    if st.button('▶ 扫描自选股', type='primary'):
        ...
        for i,code in enumerate(codes,1):
            ...
            bar.progress(i/max(len(codes),1))
        if rows:
            display_table(pd.DataFrame(rows).sort_values('抄底分',ascending=False))
        if failures:
            with st.expander('查看失败项目'):
                st.write('\n'.join(failures))
...
st.divider()
st.caption(f"北京时间：{datetime.now(CN_TZ):%Y-%m-%d %H:%M:%S} ｜ V5 双数据源云端版")
